...
export default class Users extends Component {
    ...
    submitHandler = e =>{
        ...
        axios
            .post('https://coronawatch.herokuapp.com/api/users/adminsign/',this.state)
            .then(response => {
    ...
    componentDidMount(){
        //get list of users
        const token = localStorage.getItem("login")
        axios.get('https://coronawatch.herokuapp.com/api/users/', {
            headers: {
    ...
    render() {
       if(this.state.loggedIn ===false){
        ...
        const {users_list,user_type,username,password,password2, email, first_name, last_name} = this.state
        return (
            <div>
                    ...
                    <div className="card">
                        ...
                        <button id="add_user" type="button" className="btn  btn-outline-light" data-toggle="modal" data-target="#exampleModal" style={buttonStyle} >
                            ...
                            <div className="input-group mb-3">
                                ...
                                <input id="password2"  type="password" name="password2"  className="form-control" placeholder="Confirm Password"  value={password2} onChange={this.changeHandler}/>
                            </div>
                            <br></br>
                            <div className="row">
                                {/* /.col */}
                        ...
                        <table id="example1" className="table table-bordered table-striped">
                            ...
                            {
                                users_list.length ? 
                                users_list.map(post =>  { return post.user_type===1 ? 
                                    <tr key={post.id}>
                                        <td>{post.username}</td>
                                        <td>{post.email}</td>
